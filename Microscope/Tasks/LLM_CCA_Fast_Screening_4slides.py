...
import time
import warnings
from Microscope.Logical_library.VM_CCA_Screening import CCAPredictor10, CCAPredictor20
...
def output_message(positive_num, positive_list):
...
def VLT_CCA_Fast_Screening_4slides():
    ...
    for s in range(plf_flux):
        ...
        for c in post_pt:
            ...
            while (cx <= cy+100000):
                ...
                cy = cy + step_y
                time.sleep(0.5)
        ap10 = pp/positive_num20
        if ap10 >= 0.95:
            positive_list.append(s)

    message = output_message(pos_all, positive_list)
...
if __name__ == '__main__':
    ...
    VLT_CCA_Fast_Screening_4slides()
